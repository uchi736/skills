...
description: |
  ...
  TRIGGERS (JA): ARA化して, ARAにして, 研究パッケージにして, 論文一式を作成, ARA形式に変換,
  エージェント向け研究パッケージ, 図表データを抽出して
...
metadata:
  author: ara-commons
  upstream: "github.com/AmberLJC/Agent-Native-Research-Artifact (MIT License)"
  installed: "2026-06-12, arXiv:2604.24658 のレビューを契機に公式実装を導入"
...
`/mnt/skills/user/ara-compiler/references/figure-extraction-guide.md`.
...
`/mnt/skills/user/ara-compiler/references/ara-schema.md` for field-level format.
...
Load on demand:
- `/mnt/skills/user/ara-compiler/references/ara-schema.md` — field-level format for every file
- `/mnt/skills/user/ara-compiler/references/exploration-tree-spec.md` — exploration tree YAML spec
- `/mnt/skills/user/ara-compiler/references/validation-checklist.md` — all Seal Level 1 checks
- `/mnt/skills/user/ara-compiler/references/figure-extraction-guide.md` — reading plots/diagrams/samples + PyMuPDF render/crop recipes; load when an input has figures whose information is only visual

---

## Local environment notes（2026-06-12 インストール時に追記。上流リポジトリには存在しない節）

- 出力先の既定はこの環境では `/mnt/user-data/outputs/<paper-slug>-ara/` とする（`./ara-output/` ではなく）。作業は `/home/claude` で行い、完成品のみ outputs へ
- 入力PDFは通常 `/mnt/user-data/uploads/` にある
- PyMuPDF 等の導入は `pip install --break-system-packages` を使う
- 本文はClaude Code向けの記述だが、ツールは等価に読み替える（Read→view, Write→create_file, Edit→str_replace, Bash→bash_tool。Taskは無いので逐次実行）
- PDF単体入力では src/ と trace/ が薄くなるのは設計どおり（graceful degradation）。コードリポジトリ・実験ログ・メモがあれば一緒に渡すと層が埋まる
- 1本の変換は生成→検証→修正を2〜3周するため相応に重い。AIDB記事執筆そのものには過剰で、再現・拡張・構造化抽出が目的のときに使う